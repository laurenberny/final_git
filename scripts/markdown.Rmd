...
<!-- Peer review comment: I wonder why you only referred three packages. It'd be a good idea to either comment out packages you aren't using, or referring to all packages you use  -->
...
library(ggridges)
#library(party)
#library(dplyr) Peer review comment: Commenting this out as dplyr is part of tidyverse
library(kableExtra)
library(tinytex)
#library(ggparty)
```
...
#Peer review comment: I appreciate that your team has converted these to factors, it's definitely a best practice!

#Just an idea on how to convert these to factors at once, using lapply() - this inbuilt function apply the same treatment to all elements of a vector. I checked the below code and it works fine!

#cols <- c("gender", "marital_status", "category", "class", "survived", "embarked", "disembarked")
...
# Peer review comment: I noticed in your comment below that you want the labels in the legend to come in this order: Saved, Lost. One approach is to add this code `limits = c("Saved", "Lost")` in the scale_fill_manual.
# scale_fill_manual(name = "Survival", values = c("black","dark red"), limits = c("Saved", "Lost"))
# This worked, but you'll accordingly have to change the colors you assigned as Saved was coming in black and lost in dark red with this.
...
# Peer review comment: these bunch of graphs looked very cool!
...
#Peer review comment: When I inspected the str(dat), I noticed that the factor level for survived is lost, saved. Maybe re-assigning factor levels will solve this problem you encountered. 

plot(ctree) 

#Peer review comment: The tree plot was awesome, and a great way to comprehend information. I'd definitely be saving this for future use.
...
## Calculate inflation
```{r}
#Peer review comment: This section was interesting! I'd love to know where you pulled the inflation details from. :) 
p1921 <- (17.9/9.7)
...
firstavg1912 <- "$150.00" #Peer review comment: This number came out to be 1150 for me. You can also do mean(fares$fare_1912) for reproducibility. 

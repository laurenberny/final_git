...
dat %>% 
 ...
 kable_classic(full_width = F, html_font = "Cambria")
```
Furthermore, age was also an important factor that contributed to survival as shown in the figure below.
...
surv_ageclass_hist <- surv_ageclass_hist + scale_fill_manual(name = "Survival", values = c("black","red"))
```
...
For this project, we used several packages. These included Rio  @riopackage, Readr @readrpackage, Here @herepackage, Tidyverse @tidyversepackage, Janitor @janitorpackage, Plotly @plotlypackage, Kableextra @kableExtrapackage, Ggridges @ggridgespackage, and Party @partypackage. The statistical computing software we used is R @rsoftware.
